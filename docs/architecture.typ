...
- В проекте: NPC `level/npc.tscn` (корень — `DialogueTrigger`), в `world.tscn` — старик с `dialogues/old_man.dlg`, может открыть дверь через `do node("Door").set_active(true)`.
